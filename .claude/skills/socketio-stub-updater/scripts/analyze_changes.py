...
"""Analyze changes between python-socketio versions for stub updates.

This script compares two versions of python-socketio and outputs a structured report
...
def analyze_module_changes(
    ...
    raw_name = (
        module_path.replace("src/socketio/", "").replace(".py", "").replace("/", ".")
    )
    module_name = "socketio" if raw_name == "__init__" else f"socketio.{raw_name}"
...
def get_python_files(repo_path: Path, version: str) -> list[str]:
    """Get list of Python files in python-socketio at a specific version."""
    try:
        result = subprocess.run(
            ["git", "ls-tree", "-r", "--name-only", version, "src/socketio/"],
            cwd=repo_path,
            capture_output=True,
            text=True,
            check=True,
...
def format_report(changes: list[Change]) -> str:
    ...
    lines = ["# Python-SocketIO API Changes Report", ""]
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Analyze python-socketio API changes between versions"
    )
    parser.add_argument(
        "--old-version", required=True, help="Old version tag (e.g., v5.10.0)"
    )
    parser.add_argument(
        "--new-version", required=True, help="New version tag (e.g., v5.11.0)"
    )
    parser.add_argument(
        "--socketio-path", required=True, help="Path to python-socketio git repository"
    )
    parser.add_argument(
    ...
    repo_path = Path(args.socketio_path)
